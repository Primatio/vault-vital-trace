# Timestamp Synchronization Validation Script

**Project:** Primatio rPPG Collector  
**Purpose:** Evaluate how accurately the video timestamps are aligned with Polar H10 data (ECG / RR / HR).  
...
Given a session folder produced by the Flutter app, this script:

1. Loads the video and Polar data
2. Extracts a simple rPPG signal from the face region of the video
3. Computes the optimal time lag via normalized cross-correlation
4. Reports quantitative metrics (lag, correlation, drift)
5. Generates diagnostic plots

This allows us to answer:  
> “Is the timestamp synchronization accuracy high enough for training rPPG models?”
...
### Expected `polar_data.csv` columns (minimum)

| Column         | Description                          | Required |
|----------------|--------------------------------------|----------|
| `timestamp_ms` | Milliseconds from session start or absolute | Yes     |
| `ecg_uv`       | ECG value in microvolts              | Preferred |
| `rr_ms`        | RR interval in ms                    | Alternative |
| `hr_bpm`       | Heart rate                           | Fallback |

The script prioritizes `ecg_uv` → `rr_ms` → `hr_bpm`.
...
```bash
pip install numpy pandas scipy opencv-python matplotlib librosa
# Optional but recommended for better face ROI:
...
def extract_ground_truth(df: pd.DataFrame, target_fs: float = 30.0):
    ...
    if "ecg_uv" in df.columns and df["ecg_uv"].notna().sum() > 10:
        ...
        # Simple trick: create impulses at RR times (very rough)
        sig = np.zeros(len(df))
        # Better: just use HR for now if only RR available
        sig = df["hr_bpm"].fillna(method="ffill").values.astype(float)
        name = "HR (from RR)"
...
```bash
python validate_sync.py /path/to/session_20260808_143022_subject01
```

Optional arguments:
```bash
python validate_sync.py ./my_session --fs 30 --max-lag 1.5
```
...
- Console report with lag (ms), correlation, and drift estimate
- `sync_diagnostic.png` — three plots:
  1. Original signals
  2. Signals after alignment
  3. Lag stability over time
- `sync_summary.json` — machine-readable results

---

## 7. Interpretation Guide

| Lag (absolute) | Drift (std) | Quality      | Action |
|----------------|-------------|--------------|--------|
| < 50 ms        | < 30 ms     | Excellent    | Ready for training |
| 50 – 150 ms    | < 60 ms     | Good         | Usable |
| 150 – 300 ms   | < 100 ms    | Acceptable   | Investigate |
| > 300 ms       | any         | Poor         | Fix timestamping |

---

## 8. Next Improvements (Future)

- Replace simple green-channel average with POS or CHROM
- Use MediaPipe / face detection for accurate ROI
- Support ECG peak detection + IBI comparison
- Batch mode for multiple sessions
- Automatic suggestion of a global offset correction to apply in the training pipeline
